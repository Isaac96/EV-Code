...
void CHADEMO::loop()
{
  ...
  if (bChademoMode)
  {
    ...
    if (bChademoSendRequests && bChademoRequest)
    {
      ...
      if (settings.debuggingLevel > 1) Serial.println(F("Tx"));
    }
...
void CHADEMO::doProcessing()
{
  ...
  if (/*chademoState == RUNNING && */((CurrentMillis - lastBMSTime) >= lastBMSTimeout)){
    Serial.println(F("BMS comm fault! Commencing emergency shutdown!"));
    chademoState = OPEN_CONTACTOR;
  }
...
void CHADEMO::handleCANFrame(CAN_FRAME &frame)
{
  ...
  if (frame.id == EVSE_STATUS_ID)
  {
    ...
    if (settings.debuggingLevel > 1)
    {
      ...
      Serial.print(evse_status.presentCurrent);
      Serial.print(F(" Car measured current: "));
  ...
  if (frame.id == BMS_ID) {
    lastBMSTime = millis();
    bms_status.status = frame.data.byte[0];
    if ((bms_status.status & 0x07) != 0) {
      if ((bms_status.status & BMS_STATUS_HVC) != 0) {
        carStatus.battOverVolt = 1;
        chademoState = CEASE_CURRENT;
      }
      if((bms_status.status & BMS_STATUS_BVC) != 0){
        Serial.println(F("Reached balance threshold"));
      }
    }
  }
  checkRAM();
